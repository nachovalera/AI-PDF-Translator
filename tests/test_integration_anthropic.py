"""Integration tests that exercise the real Anthropic API."""
...
from ai_pdf_translator.provider import AnthropicProvider


pytestmark = pytest.mark.skipif(
    not (os.getenv("ANTHROPIC_API_KEY") and os.getenv("RUN_ANTHROPIC_TESTS")),
    reason="Integration tests require ANTHROPIC_API_KEY and RUN_ANTHROPIC_TESTS=1",
)
...
def test_translate_chunk_real_anthropic():
    translation_service.set_provider(AnthropicProvider())
    try:
...
@pytest.mark.integration
def test_translate_pdf_real_anthropic(tmp_path):
